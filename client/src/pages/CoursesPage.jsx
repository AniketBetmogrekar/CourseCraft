...
export default function CoursesPage({ token, onMessage }) {
  ...
  async function saveCourse(values) {
    if (!token) {
      ...
      onMessage("Sign in as an instructor to manage courses.");
      navigate("/login");
      return;
    }
    setBusy(true);
    ...
    <>
      <section className="hero">
        <div>
          ...
          <p>Thoughtful courses for wherever you want to go.</p>
          <button
            className="primary"
            onClick={() =>
              document
      ...
      <section className="section" id="course-library">
        <div className="section-heading">
          <div>
            <span className="eyebrow">A PATH THAT’S YOURS</span>
        ...
        {filtered.length ? (
          ...
          <div className="empty-state">
            <h3>
              {courses.length ? "No matching courses." : "No courses yet."}
            </h3>
            <p>
              {courses.length
